...
GLuint GlobalParam::	_iTexFormat = GL_RGBA32F;	//or GL_RGBA16F_ARB
...
void GlobalUtil::InitGLParam(int NotTargetGL)
{
    ...
    }else
    {
	    ...
	    if(vendor)
	    {
			...
			}else if(strstr(vendor, "ATI") && false)
			{
	    ...
	    if (ogl_ext_ARB_fragment_shader == ogl_LOAD_FAILED ||
		...
		if (ogl_ext_EXT_framebuffer_object == ogl_LOAD_FAILED) 
	    {
		    std::cerr << "Framebuffer object not supported!\n";
		    GlobalUtil::_GoodOpenGL = 0;
	    }

		if(ogl_ext_ARB_texture_rectangle != ogl_LOAD_FAILED)
	    {
	        GLint value;
		...
		GlobalUtil::_SupportNVFloat = ogl_ext_NV_float_buffer == ogl_LOAD_SUCCEEDED;
		GlobalUtil::_SupportTextureRG = ogl_ext_ARB_texture_rg == ogl_LOAD_SUCCEEDED;
