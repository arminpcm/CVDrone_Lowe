...
void PyramidGL::GenerateFeatureDisplayVBO()
{
	...
	for(int i = 0, idx = 0; i < _octave_num; i++)
	{
		for(int j = 0; j < param._dog_level_num; j ++, idx++)
		{
			...
			glBindBuffer(GL_PIXEL_PACK_BUFFER,  _featurePointVBO[ idx]);
			glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize);
			esize = ftex->GetImgHeight() * ftex->GetImgWidth()*sizeof(float) *4;

			//increase size when necessary
			if(bsize < esize)
			{
				glBufferData(GL_PIXEL_PACK_BUFFER, esize*3/2 ,	NULL, GL_STATIC_DRAW);
				glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize);
			}

			//read back if we have enough buffer
			if(bsize >= esize) glReadPixels(0, 0, ftex->GetImgWidth(), ftex->GetImgHeight(), GL_RGBA, GL_FLOAT, 0);
			else  glBufferData(GL_PIXEL_PACK_BUFFER, 0,	NULL, GL_STATIC_DRAW);
			...
			//
			glBindBuffer(GL_PIXEL_PACK_BUFFER, _featureDisplayVBO[ idx]);
			glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize);
...
void PyramidGL::ReshapeFeatureListCPU()
{
	...
	glBindBuffer(GL_PIXEL_UNPACK_BUFFER,  ListUploadPBO);
	glBufferData(GL_PIXEL_UNPACK_BUFFER, szmax * 8 * sizeof(float),	NULL, GL_STREAM_DRAW);
#endif
...
#else
        float* mem = (float*) glMapBuffer(GL_PIXEL_UNPACK_BUFFER,  GL_WRITE_ONLY);
